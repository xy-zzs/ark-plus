...
public class WebAutoConfiguration {
  ...
  /**
   * 聚合所有模块 i18n 资源。 各模块 *-spring-boot-starter 按约定在 resources/i18n/{module}/ 下放置文件即可。 新增模块时在此追加对应
   * basename。
   */
  @Bean
  public MessageSource messageSource() {
    ...
    source.setBasenames(
        "classpath:i18n/messages", "classpath:i18n/auth/messages", "classpath:i18n/user/messages");
  ...
  /**
   * Locale 解析策略：优先 lang 参数，其次 Accept-Language Header Spring MVC 会用此 Bean 在请求进入时写入
   * LocaleContextHolder， 保证 MessageSourceHolder 和 MessageSourceResolver 都能拿到正确 Locale
   */
  @Bean
  ...
  // ------
  /**
   * MessageSource：扫描所有模块的 i18n/messages 文件 启动模块放 messages.properties，各业务模块也可放自己的消息文件， 通过
   * addBasenames 叠加，互不覆盖
   */
  /*@Bean
  public MessageSource messageSource() {
  ...
  /**
   * LocaleResolver：从 Accept-Language 请求头解析 locale 前端请求时携带 Accept-Language: en 即切换为英文 默认 locale 为中文
   */
  /*@Bean
  @ConditionalOnMissingBean
  public LocaleResolver localeResolver() {

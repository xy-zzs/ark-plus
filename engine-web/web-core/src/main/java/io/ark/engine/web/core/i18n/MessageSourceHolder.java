 ...
 *
 * <p>适用场景：工具类、枚举、静态方法中需要获取 i18n 文案时。 能注入 Bean 的场景优先使用 {@link MessageResolver}，更易测试。
 *
 * <p>由 {@code WebAutoConfiguration} 在 MessageSource Bean 初始化后调用 {@link #setMessageSource}
 * 完成注入，应用启动后即可使用。 @Author: Noah Zhou
 */

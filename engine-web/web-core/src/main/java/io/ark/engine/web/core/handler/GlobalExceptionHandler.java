 ...
 *
 * <ol>
 *   <li>参数校验异常 — 400，返回具体字段错误信息
 *   <li>ArkException 及其子类 — 使用异常自带的 code 和 message
 *   <li>Spring MVC 标准异常 — 映射到对应 HTTP 语义
 *   <li>未知异常 — 500，隐藏内部细节，只记日志
 * </ol>
 *
 * @Author: Noah Zhou
 */
...
public class GlobalExceptionHandler {
  ...
  public Result<Void> handleNotReadable(HttpMessageNotReadableException ex) {
    ...
    String i18n =
        messageSource.getMessage(
            WebErrorCode.BAD_REQUEST.getMessageKey(), null, LocaleContextHolder.getLocale());
    return Result.fail(WebErrorCode.BAD_REQUEST.getCode(), i18n);
  }
  ...
  public Result<Void> handleNotFound(NoResourceFoundException ex) {
    ...
    return Result.fail(
        WebErrorCode.NOT_FOUND.getCode(),
  ...
  public Result<Void> handleMethodNotAllowed(HttpRequestMethodNotSupportedException ex) {
    ...
    return Result.fail(
        WebErrorCode.METHOD_NOT_ALLOWED.getCode(),
  ...
  public Result<Void> handleUnknown(Exception ex) {
    log.error("未知异常", ex);
    return Result.fail(
        WebErrorCode.INTERNAL_ERROR.getCode(),

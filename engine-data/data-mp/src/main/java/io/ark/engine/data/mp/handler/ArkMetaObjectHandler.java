 ...
 *     <p>依赖 Spring Security 上下文获取当前用户 ID。 若上下文为空（定时任务、初始化脚本等场景），createdBy/updatedBy 填 0L。
 * @author Noah Zhou
 */
@Slf4j
@Component
public class ArkMetaObjectHandler implements MetaObjectHandler {
   ...
   *
   * <p>约定：SecurityUser（在 security-core 里定义）实现 UserDetails， 其 principal 可转为 Long userId。 此处用 Object
   * 接收，避免 data-mp 直接依赖 security-core。
   */
  private Long resolveCurrentUserId() {
    try {
      ...
      log.debug(
          "MetaObjectHandler: unrecognized principal type [{}], fallback to SYSTEM_USER_ID",
